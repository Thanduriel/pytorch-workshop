...
\begin{frame}{Ingredients}
	\centering
	\begin{tikzpicture}
		\node(problem)[Box, label=below:Problem]{\includegraphics[height=0.8cm]{data.png} {\Huge $\rightarrow$} \includegraphics[height=0.8cm]{data.png}};
		...
		\node(optimizer)[Box,  label={[trans on=<3->]below:Optimizer}, trans on=<3->, right= of neuralNetwork]{{\Huge $\mathcal{L}$}}; % \includegraphics[height=1cm]{loss.png}
...
\begin{align*}
	g_j(x) &= \sigma (W_j x + b_j) ,\quad j = 1,\dots,\,L-1\, \\
	g_L(x) &= W_L x + b_L.
\end{align*}
...
\begin{frame}[fragile]{Getting Started}
	...
	\begin{itemize}
		...
		\begin{minted}{bash}
git clone https://github.com/Thanduriel/pytorch-workshop.git
		\end{minted}
...
\begin{frame}{Regularization Techniques}
	...
	\[
	\hat{\mathcal{L}} (u, \hat{u}, w_r) = \mathcal{L} (u,\hat{u}) + \lambda ||w_r||_2^2
	\]
	\visible<2->{
	Layer normalization
	\[
